...
export default function AIProvidersTab() {
  ...
  const handleAddKey = async () => {
    ...
    const { data, error } = await (supabase.from("ai_api_keys") as any).insert({
      provider_id: keyDialog.providerId,
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="font-display font-semibold">AI Providers & API Keys</h3>
          <p className="text-xs text-muted-foreground">Manage multiple AI providers with round-robin API key rotation</p>
        </div>
      </div>
            ...
            <div key={provider.id} className={`rounded-2xl border-2 bg-card p-5 transition-all ${provider.is_default ? "border-primary/40 bg-primary/5" : provider.is_active ? "border-border" : "border-border opacity-50"}`}>
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-3">
                  <div className={`p-2 rounded-xl ${provider.is_default ? "gradient-bg" : "bg-muted"}`}>
                    <Settings2 className={`h-4 w-4 ${provider.is_default ? "text-primary-foreground" : "text-muted-foreground"}`} />
                  </div>
                  <div>
                    <div className="flex items-center gap-2">
                      <h4 className="font-display font-bold text-sm">{provider.display_name}</h4>
                      {provider.is_default && (
                        <span className="px-2 py-0.5 rounded-full text-[10px] font-bold gradient-bg text-primary-foreground">DEFAULT</span>
                      )}
                    </div>
                    <p className="text-[11px] text-muted-foreground">{activeKeys} active key{activeKeys !== 1 ? "s" : ""} • {(provider.models || []).length} models</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <Button size="sm" variant="outline" className="gap-1 text-xs" onClick={() => setEditingProvider({ ...provider })}>
                    <Edit className="h-3 w-3" /> Edit
                  </Button>
                  <Button size="sm" variant="outline" className="gap-1 text-xs" onClick={() => setKeyDialog({ open: true, providerId: provider.id, providerName: provider.display_name })}>
                    <Key className="h-3 w-3" /> Keys ({providerKeys.length})
                  </Button>
                  {!provider.is_default && (
                    <Button size="sm" variant="outline" className="gap-1 text-xs" onClick={() => handleSetDefault(provider)}>
              ...
              <div className="flex flex-wrap gap-1.5">
                {(provider.models || []).map((m: AIModel) => (
                  <span key={m.id} className="px-2 py-0.5 rounded-full text-[10px] font-medium bg-muted text-muted-foreground">
                    {m.name}
                  </span>
                ))}
              </div>
      ...
      <Dialog open={keyDialog.open} onOpenChange={(v) => setKeyDialog(prev => ({ ...prev, open: v }))}>
        <DialogContent className="max-w-lg max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="font-display">API Keys — {keyDialog.providerName}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            {/* Add new key */}
            <div className="p-4 rounded-xl bg-muted/50 border border-border space-y-3">
              ...
              <Button size="sm" className="gradient-bg border-0 text-primary-foreground gap-1" onClick={handleAddKey}>
            ...
            <div className="space-y-2">
              {apiKeys.filter(k => k.provider_id === keyDialog.providerId).map(key => (
                <div key={key.id} className={`flex items-center justify-between p-3 rounded-xl border ${key.is_active ? "border-border bg-card" : "border-destructive/20 bg-destructive/5 opacity-60"}`}>
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-muted-foreground mt-0.5">
                      Used {key.usage_count}x {key.last_used_at ? `• Last: ${new Date(key.last_used_at).toLocaleDateString()}` : ""}
                    </p>
                  </div>
                  <div className="flex items-center gap-1.5 ml-2">
                    <Button size="sm" variant="ghost" className="h-7 w-7 p-0" onClick={() => setShowKeys(p => ({ ...p, [key.id]: !p[key.id] }))}>
                    ...
                    <Button size="sm" variant="ghost" className="h-7 w-7 p-0 text-destructive" onClick={() => handleDeleteKey(key.id)}>
              ...
              {apiKeys.filter(k => k.provider_id === keyDialog.providerId).length === 0 && (
                <p className="text-xs text-muted-foreground text-center py-4">No API keys yet. Add one above.</p>
              )}
            </div>
          </div>
        </DialogContent>
      ...
      <Dialog open={!!editingProvider} onOpenChange={() => setEditingProvider(null)}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle className="font-display">Edit Provider</DialogTitle>
          </DialogHeader>
          {editingProvider && (
            <div className="space-y-4">
              <div>
              ...
              <div>
                ...
                <Input value={editingProvider.base_url} onChange={e => setEditingProvider(p => p ? { ...p, base_url: e.target.value } : null)} className="mt-1" />
              </div>
              <div>
                <Label className="text-xs text-muted-foreground">Models (JSON)</Label>
                <textarea
                  className="mt-1 w-full rounded-md border border-border bg-background px-3 py-2 text-sm font-mono min-h-[120px]"
                  value={JSON.stringify(editingProvider.models, null, 2)}
                  onChange={e => {
                    try {
                      const parsed = JSON.parse(e.target.value);
                      setEditingProvider(p => p ? { ...p, models: parsed } : null);
